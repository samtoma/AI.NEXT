...
const ALL_SQL = {
  ...
  histogram: STUDENT_DAY_HISTOGRAM_SQL,
...
test("the window sees the day before the period, and the period is applied after it", () => {
  for (const sql of [STUDENT_DAY_HISTOGRAM_SQL, STUDENT_DAYS_SQL]) {
    const s = squash(sql);
...
test("readUploadsView: four queries, this environment, this period, the threshold from the module", async () => {
  const { db, calls } = fakeDb([
    [{ uploads: "23", students: "3" }],
    [
    ...
    [
      { most: "1", student_days: "2" },
      { most: "10", student_days: "1" },
      { most: "12", student_days: "1" },
    ],
    [
  ...
  const view = await readUploadsView(db, 30);

  assert.deepEqual(
    calls.map((c) => c.text),
    [UPLOAD_TOTALS_SQL, PARSE_OUTCOMES_SQL, STUDENT_DAY_HISTOGRAM_SQL, STUDENT_DAYS_SQL]
  );
  assert.deepEqual(calls[0].values, [ENVIRONMENT, "30"]);
  assert.deepEqual(calls[1].values, [ENVIRONMENT, "30"]);
  assert.deepEqual(calls[2].values, [ENVIRONMENT, "30"]);
...
test("no uploads at all reads as zeros the page can tell apart from data", async () => {
  const { db } = fakeDb([[{ uploads: 0, students: 0 }], [], [], []]);
  const view = await readUploadsView(db, 7);
...
test("the list says when it was cut", async () => {
  ...
  const view = await readUploadsView(db, 90);
  assert.equal(view.recentDays.length, UPLOAD_DAYS_LIMIT);
...
test("the Cost page: the upload panel sits beside the turn-limit panel, honest when empty, amber when reached", () => {
  ...
  assert.match(cost, /\(\) => readUploadsView\(db, periodDays\),\s*\] as const\)/);
  const page = code("app/(console)/cost/page.console.tsx");
