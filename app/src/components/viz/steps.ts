 ...
 *
 * geo_scene / arrow_map carry REAL step semantics (element `step` fields /
 * one arrow per step); every other kind is mapped to thirds of its natural
 * timeline by useVizTimeline's fraction window. Must stay consistent with
 * the stepTimes arrays the two step-aware primitives pass to useVizTimeline
 * (a filtered-out malformed element can at worst over-count a dot — the
 * playback window itself always follows the primitive's own stepTimes).
 */
