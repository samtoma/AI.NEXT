...
- **Cost:** no numeric ceiling binds until PRD §10 sets a price (the EGP 40 figure came from a withdrawn parent price band). Per-student spend instrumentation remains mandatory; upload/OCR cost is metered separately. **Since 2026-09-24 (ADR-0023, constitution v3.3.0) the per-surface turn caps no longer bind:** no surface refuses a turn for reply count; the same numbers survive as observed thresholds, counted per environment and shown in the console, so worst-case spend per conversation is unbounded by the server and watched instead.
