...
| FR-4308 | App notation; vocabulary and contexts as printed; refutations cite this book only | **OPEN** | planned: assembly normalisation (`align*` written as `aligned` for inline KaTeX) | coverage normalisation counts; `test_lesson_collect2.py` | T338 |
...
| FR-4407 | Maths images turned into checked text: two independent passes, hash or agreement, PDF cross-check, queue for a human, never guessed | **OPEN** | planned: `transcribe-maths.workflow.js`, `assemble_maths.py` (B21; an aligned derivation's hash form proven, stored with a real `&`) | `test_assemble_maths.py`, `test_lesson_collect2.py`; S0b counts (SC-213) | T418–T421 |
