...
- [X] T047 [US3] Meter upload parsing as its own `surface_kind` in `ai_interactions` and enforce a per-student daily cap (10) — image tokens are materially more expensive and must not hide inside a blended per-student figure (Principle VI) *(Cap clause superseded 2026-09-24 — ADR-0023, 002 FR-3407/FR-3408: no upload is refused for daily count any more; the 10 survives as an observed threshold, `DAILY_UPLOAD_THRESHOLD`. The metering half — its own `surface_kind`, kept separate from tutoring spend — stands unchanged and is what FR-3409's Cost-page monitoring is built on.)*
