...
**Status date**: 2026-09-24 (rev. 13 — §9 items 16 and 17 resolved by spec 002's FR-3201…FR-3210; an evidence note on FR-1108) · **Branch**: `feat/002-identity-and-admin-console`
...
|---|---|---|---|---|
| FR-C01 | Grounded teaching — never solved from scratch | **BUILT** | `app/src/lib/retrieval.ts`, `lib/ask.ts`, `lib/lesson.ts` (T033–T034) | Prompt-capture harness (T037) needs a live DB; verified by deliberate diff review instead — the refactor touches ledger inserts and appends a retrieval block that renders to `""` when nothing is retrieved · **2026-09-26 (003, Samuel's G2 answer 22, decision 43; status unchanged)**: a question flagged `choices.answer_only` (the book prints no working) reaches the tutor with an instruction to confirm, give the answer and point to the lesson's worked examples — never its working and never a method of the model's own — on the lesson question bank, the Ask context's question in scope and the Socratic probe note (`lib/question-flags.ts`; `answer-only-prompts.test.mts`, `question-flags.test.mts`) |
...
| FR-1108 | Live-unreviewed count reportable and disclosed with every parity result | **VERIFIED** | Disclosed on every `parity_check.py` run **and** on `/admin/content` | Page renders live: 450 book · 12 generated unchecked · 0 checked · 0 awaiting promotion. **Note 2026-09-24:** the console's Content page now counts per subject — a switcher, a "held" tile, every count computed from the rows shown (`a809e90`); that behaviour is its own requirement, **FR-3212** in spec 002 (BUILT: counts checked on production, page not viewed signed in). This row's status is unchanged |
...
| FR-1206 | A diagnosis, not a score | **VERIFIED** | the `onResult` notes; `widgets/number-line-grade.ts` (v0.9.3) | Fired live: *"that is the MEDIAN… the mean is the total shared out equally"*; *"inverse means the product stays the same: 6 × 10 = 60, but 4 × 7 = 28"*; *"your stroke doubles back… it is not the graph of a function at all"*. **Amended v0.9.3 (2026-09-25, hotfix, not yet released):** `number_line_marker` named the wrong error for a sign slip — marking −2 and 3 for 1/((x − 2)(x + 3)) reported `missed-values`, whose refutation opens *"the value you found genuinely does break the fraction"*, to a student who had found neither. Its points-mode grading moved into the pure `number-line-grade.ts` and gained **`sign-flipped`** (every wrong mark is the negative of a missed value), checked before the omission; the three `domain-excluded` questions map it to `mc:u1-1-1:transposition-sign`, a prerequisite's misconception (FR-1215, checked against the graph). `widget-number-line-grade.test.mts` (7): the sign error on each stored row diagnosed and mapped to its refutation, one sign of two flipped, the omission and the extra still told apart, zero and symmetric sets. **Gap: the new note has not been rendered in a browser or seen by the tutor.** |
| FR-1207 | Reject rather than repair; reject the unreachable | **VERIFIED** | `lib/widget-payloads.ts`; for the stored bank, `lib/widget-bank-stems.test.mts` (v0.9.3) | 5 unreachable-but-well-typed payloads render nothing, shown as a panel on the fixture: string coordinates, a 37° angle off the 5° snap, a fourth term of 40/3, sin θ = 1, a quadratic with a = 0. **Amended v0.9.3 (2026-09-25, hotfix, not yet released) — the rule was held for payloads a model composes and never for the stored bank, and the stored bank broke it.** Three live widget questions, `q:t2u2-2-1:w001–w003` ("Mark every value x cannot take in 1/((x − 2)(x + 3))" and two like it), stored the NEGATIVES of their excluded values as the answer key, so a correct answer was marked wrong — and the wrong set drawn on the line as the answer — while the sign error was marked right. Cause: the generator's `domain-excluded` template stored the factors' shifts, not their roots; its stems and solutions were right. Production held **0 attempts** on them (read-only check). Fixed in the template, the seed and, for the database production already has, **migration 032** (idempotent; only these three ids, only while they hold the wrong set; rollback provided; floor raised to 31). `widget-bank-stems.test.mts` (7) reads **every** stored widget's stem blindly (the denominator's factors, the line's coefficients, the proportion's terms, …), derives the spec it asks for and holds the stored key and the canonical solution's numbers to it — all 48 agree, and on v0.9.2's seed it fails on exactly these three — and runs every stored spec through `parseMathWidget`; it also pins 032's values to the seed's. `services/extraction/tests/test_widget_stems.py` (5) holds the generator to the same reading. `widget-excluded-values-db.test.mts` (4, opt-in, run once): 032 changes the three rows only, a re-run leaves every `xmin` unchanged, the rollback restores the pre-032 rows exactly, a hand-corrected row is left alone. `scripts/ci-migrations.sh all` against **v0.9.2**: all checks passed. On a scratch copy of the local database: 032 fixed the three rows and no other (2,108 other question rows byte- and `xmin`-identical), a second deploy and a v0.9.2 redeploy wrote nothing, the rollback restored the old rows exactly, HEAD re-applied, the whole widget bank then equalled the seed; `parity_check.py`: **PARITY: GREEN**. **Gaps: production keeps the wrong keys until a deploy runs 032; the corrected widgets have not been answered in a browser.** |
...
| 9 | ~~The Socratic protocol has no requirement either~~ — **resolved 2026-09-20**, Samuel approved. Written as **FR-209**…**FR-213**, all BUILT | Samuel | Stated as obligations on the product, never as prompt wording — a prompt is an implementation of these and will be rewritten many times. **Note 2026-09-24**: FR-209…FR-213 state the general elicit-before-explaining rhythm; **Socratic PROBING** — the wrong-answer mechanism that holds a card's answer back for a guiding question first (`wip/socratic-probing-route-b`, [ADR-0021](../../docs/decisions/0021-runtime-teaching-toggle-and-testers.md)) — is a separate, newer behaviour that stayed entirely unrequired beyond its console switch (002 `FR-3101`…`FR-3111`, which govern who may turn probing on and for whom, not what it does to a wrong answer once on). **One rule of probing's own behaviour now has a requirement**: 002 `FR-3112`, set by Samuel 2026-09-24 — the answer and worked solution are withheld until the student's second wrong attempt, and neither a request nor any tutor directive shortens that. **The rest of probing's behaviour stays unrequired** — the guiding question's own wording, what counts as a genuine attempt, the stall risk ADR-0021's amendment names and leaves open — on `CLAUDE.md`'s own rule against inventing FRs to cover shipped behaviour; do not write more without Samuel's instruction. |
...
| 16 | ~~**Lesson progression shipped without a requirement** — the persisted per-course lesson pointer ([ADR-0020](../../docs/decisions/0020-mastery-gated-lesson-progression.md), migration 028)~~ — **resolved 2026-09-24**: written as **FR-3201…FR-3207** in [spec 002](../002-identity-and-admin-console/spec.md) ("Shipped in v0.6.0 without a requirement"), traced in [its matrix §7f](../002-identity-and-admin-console/traceability.md), on Samuel's instruction; not filed under any existing FR | **Samuel** | New 2026-09-23, arrived with Tamer's work on `main`. The original entry asked for it to be written or recorded as ADR-only, and not filed under an existing FR to make the matrix look complete — it was written as seven new requirements, from ADR-0020 and the code as it behaves, all VERIFIED on a signed-in run of v0.6.0 plus `progression.test.mts` / `student-landing.test.mts` (the rows say which evidence is which) |
| 17 | ~~**The `/spine` skill map shipped without a requirement** — the student-facing Noor Play redesign of `/spine` (`b9df1e1`)~~ — **resolved 2026-09-24**: written as **FR-3208…FR-3210** in [spec 002](../002-identity-and-admin-console/spec.md), traced in [its matrix §7f](../002-identity-and-admin-console/traceability.md) | **Samuel** | New 2026-09-23, same arrival. Two VERIFIED on a signed-in run of v0.6.0 (band words and fills with no percentage or id; the iPad-portrait docking); **FR-3210 PARTIAL** — the keyboard path was run, the Dock control was not. FR-1001's scope line, which still lists `/spine` among the internal tools, is unchanged here and is a separate edit |
