...
| 16 | **Lesson progression shipped without a requirement** — the persisted per-course lesson pointer ([ADR-0020](../../docs/decisions/0020-mastery-gated-lesson-progression.md), migration 028): `/student` opens on the lesson the student is on, which advances when every objective in it reaches 0.75 | **Samuel** | New 2026-09-23, arrived with Tamer's work on `main`. It changes what every student sees first, and no FR states it. The ADR is the decision record, not a requirement — so no test can declare it and the counts below cannot see it. The pure rules are unit-tested (`progression.test.mts`, `@covers ADR-0020`). Write it as a requirement or record that it stays ADR-only; **do not** file it under an existing FR to make the matrix look complete |
| 17 | **The `/spine` skill map shipped without a requirement** — the student-facing Noor Play redesign of `/spine` (`b9df1e1`: `SpineExplorer`, `GraphCanvas`, `LoPanel`, `NoorPanel`) | **Samuel** | New 2026-09-23, same arrival. It is a student surface in its own right — `spec.md` still lists `/spine` among the *internal tools* (FR-1001's scope line) — and the rows that touch it (FR-205's upload control, for one) cite it as evidence without any requirement saying what the surface is for. Same choice as item 16 |
